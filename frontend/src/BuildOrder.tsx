...
export const addResourcesUpToCurrentStep = (
  buildOrder: IBuildOrder,
  gameTime: number
) => {
...
export const shuffleVillagerGenders: (steps: IBuildOrderStep[]) => void = (
  steps
) => {
  steps.forEach((step) => {
...
export const getNextRelevantMoment = (
  buildOrder: IBuildOrder,
  currentTime: number
) => {
...
export const getPreviousRelevantMoment = (
  buildOrder: IBuildOrder,
  currentTime: number
) => {
  let currentIndex = getCurrentStepIndex(buildOrder, currentTime);
...
export const mergeSubsteps: (steps: IBuildOrderStep[]) => IBuildOrderStep[] = (
  buildOrderSteps
) => {
...
export const unfoldSubsteps: (
  steps: IBuildOrderStep[]
) => ISortableBuildOrderStep[] = (buildOrderSteps) => {
  const newSteps: IBuildOrderStep[] = [];
  buildOrderSteps.forEach((step) => {
    newSteps.push(step);
    if (step.subSteps) {
...
function BuildOrder() {
  const [startTime, playing, timeAlreadyPlayed, togglePlaying, updateGameTime] =
    usePlayingState();
  ...
  const steps = buildOrder.steps.map((step, index) => {
    return (
      <div className="buildorder-step-wrap" key={index}>
  ...
  return (
    <div
      className={`buildOrder ${
        showPerVillagerMarks ? "per-villager-marks" : ""
      }`}
    >
      <BuildOrderHeader
