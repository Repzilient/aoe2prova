...
) => JSX.Element | null = (hide, property, state) => {
  return hide ? null : (
    <div className={`property ${property}`}>
      <label>{property}</label>
      <BuildOrderIconSelect
        icon={state[0]}
        onIconSelect={(icon) => state[1](icon)}
      />
    </div>
  );
};
  ...
  return hide ? null : (
    <div className={`property number ${property}`}>
      ...
      <input
        ...
        step={1}
        value={state[0]}
        onChange={(e) => state[1](parseInt(e.target.value))}
      />
    </div>
  );
};
...
const EditBuildOrderStep: React.FC<IEditBuildOrderStepProps> = ({
  step,
  onEdit,
  onRemove,
}) => {
  ...
  const doneEditing = () => {
    const newStep = Object.assign(JSON.parse(JSON.stringify(step)), {
      id: step.id,
      kind: kind,
      from: fromState[0],
  ...
  const hideFrom =
    ["age2", "age3", "age4", "loom", "create", "research"].indexOf(kind) >= 0;
  const hideNumber =
    ["age2", "age3", "age4", "loom", "research"].indexOf(kind) >= 0;
  const hideTarget =
    ["age2", "age3", "age4", "loom", "research"].indexOf(kind) >= 0;
  ...
  const buildNumber = numberProperty(
    hideBuild,
    "buildAmount",
    buildAmountState
  );
  const techsList =
    kind === "research" ? (
      <ResearchList techs={techs} setTechs={setTechs} />
    ) : null;
  return (
    <div className="buildorder-step-edit">
      <Select value={kind} onChange={setKind} autoFocus optionFilterProp="text">
...
const BuildOrderStepEdit: React.FC<IBuildOrderStepEditProps> = ({
  step,
  onEdit,
  onRemove,
}) => {
  ...
  const showEdit: React.ReactElement = (
    <EditBuildOrderStep step={step} onEdit={doneEditing} onRemove={onRemove} />
  );
  const className = `buildorder-step-wrap${
    step.duringPrevious ? " substep" : ""
  }`;
  return (
