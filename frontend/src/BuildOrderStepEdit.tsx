...
function computeTrackerPosition(completion: ICompletion): [number, boolean] {
  ...
  const currentPosition = parseFloat(
    window
      .getComputedStyle(document.querySelectorAll(".buildorder-tracker")[0])
      .getPropertyValue("top")
  );
  const topLevelSteps = document.querySelectorAll(
    ".buildOrder > .buildorder-step-wrap"
  );
  ...
  const currentStepHeight = currentStepElement
    ? currentStepElement.scrollHeight
    : 0;
...
) => ICompletion = (buildOrderSteps, timeSinceStart) => {
...
const getTimeSinceStart: (
  startTime: Date | null,
  elapsedTime: number
) => number = (startTime, elapsedTime) => {
  if (!startTime) {
    startTime = new Date();
  }
  let timeSinceStart =
    (gameSpeed * (new Date().getTime() - startTime.getTime())) / 1000;
...
const speakNewStepsOutLoud = (
  ...
  if (zeroTimeStepsToDoFirst.length > 0) {
    ...
    // MODIFICA: Tradotto "Then" in "Poi"
    message += ", Poi, ";
  }
  const utterance = new SpeechSynthesisUtterance(
    message + getMessageForStep(currentStep)
  );
  // MODIFICA: Impostata la lingua su Italiano
  utterance.lang = "it-IT";
  speech.speak(utterance);
};

const handleBuildOrderStateChange = (
  currentBuildOrder: IBuildOrder,
...
const BuildOrderTracker: React.FC<IBuildOrderTrackerProps> = ({
  buildOrder,
  startTime,
  elapsedTime,
  playing,
  onNewBuildOrderState,
  gameTimeChange,
}) => {
  ...
  useEffect(() => {
    const timeout = window.setTimeout(() => {
      ...
      handleBuildOrderStateChange(
        buildOrder,
        completion,
        { readStepsOutLoud: readStepsOutLoud },
        onNewBuildOrderState
      );
      if (changed) {
        setTrackerPosition(newPosition);
        const tracker =
          document.getElementsByClassName("buildorder-tracker")[0];
        try {
          tracker &&
  ...
  return (
    <div className="buildorder-tracker" style={style}>
